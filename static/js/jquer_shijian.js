  ...
  $.fn.shijian = function (opt) {
    function cPlugin(o, emlnum) {
      ...
      sjObj.defaults = {
        ...
        finalshow: true,
        boxClassName: "",
        onfun: function onfun(sjObj) {//取消改变时间时候执行事件

        },
        okfun: function okfun(sjObj) {
          //确认时间时候执行事件
          var str = sjObj.opt.Format.replace("yyyy", sjObj.opt.yyyy);
        ...
        t_box: null, //保存生产元素盒子
        df_persp: function df_persp() {
          return $("<div class='df-persp'><div class='persp-bg'></div>");
        },
        df_box: function df_box() {
        ...
        df_main: function df_main() {
          return $("<div class='df-main'>");
        },
        df_btn: function df_btn() {
        ...
        strStart: function strStart(text, c) {
          var df = this;
          var str;
          var text = text || "";
          console.log(this);
        ...
        //返回数组组成的html字符串
        buildArrStr: function buildArrStr(Arr, txt, c) {
        ...
        //变量赋值监听
        vardata: function vardata(name, val) {
          if (!val) {
            return;
          }
        ...
        //根据时间判断当月天数
        daysJudge: function daysJudge(name) {
        ...
        moveElm: function moveElm(eml) {
          ...
          return $(eml).each(function () {
            ...
            var moveing = function moveing(e) {
              stop(e);
              if (e.originalEvent.touches) {
      ...
      var GetSlideDirection = function GetSlideDirection(startX, startY, endX, endY) {
        //判读手指滑动方向
        var dy = startY - endY;
        var dx = endX - startX;
        var result = 0;
      ...
      var getTranslateY = function getTranslateY(eml) {
