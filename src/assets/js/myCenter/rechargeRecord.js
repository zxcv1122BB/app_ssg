export default {
  ...
  mounted: function() {
    ...
    $("#section").css({
			overflow:'scroll',
			height:$("#container").height()-44-$(".chooseType").height()-10+"px"
		})
  ...
  methods: {
    ...
    getdatasMes: function() {
      ...
      if(this.page <= this.totalPage) {
        var obj = {
          ...
          data: data,
          dataType: 'json',
          url: '/authApi/bets/getNewMemberDepositRecord',
    ...
    clickMes: function(index) {
      this.rechargeDetails = this.datas[index];
      localStorage.setItem('rechargeDetails', JSON.stringify(this.rechargeDetails));
      this.$router.push({ name: 'rcdetail',})
    },
    //点击选择类型
      ...
      var toUpdate = {
        ...
        loadmore: function(obj) {
          if(finished == 0) {
            ...
            var scrollHeight = $(obj).height(); //获取内容高度。
  ...
  watch:{
    '$route'(to, from) {
      if (to.name =="rechargeRecord"){
        this.selectType('');
        $("#showType").val("全部状态");
        this.initDom();
      } else if (from.name =="rechargeRecord"){
