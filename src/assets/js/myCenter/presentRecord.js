export default {
  ...
  methods: {
    ...
    getdatasMes: function() {
      ...
      if(this.page <= this.totalPage) {
        var obj = {
          ...
          data: data,
          dataType: 'json',
          url: '/authApi/bets/getNewMemberWithdrawRecord',
    ...
    clickMes: function(index) {
      this.presentDetails = this.datas[index];
      localStorage.setItem('presentDetails', JSON.stringify(this.presentDetails));
      this.$router.push({ name: 'psdetail'})
    },
    //点击选择类型
  ...
  watch:{
    '$route'(to, from) {
      if (to.name =="presentRecord"){
        this.selectType('');
        $("#showType").val("全部状态");
        this.initDom();
      } else if (from.name =="presentRecord"){
