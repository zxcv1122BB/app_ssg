...
//			baseIndex: 1,
  ...
  created: function () {
    ...
    if(localStorage.appDownloadShow == 2){
      ...
      _this.appDownloadShow = true;
		...
		// setTimeout(function(){
			...
			$('#push .mui-icon-close').on('tap', function() {
				mui('#push').popover('hide');
			});
			
			$('#push .up').on('tap', function() {
				mui('#push').scroll().scrollTo(0, 0, 500);
			});
	// },3000)
	...
	methods: {
		...
		showHotArea(index,tit){
				this.hot_index=index;
				this.hot_title=tit;
			// $('.move').on('click', function() {
				mui('#push').popover('show');
				// setTimeout(function(){
          ...
          success: function(data) {
            if(data.code == 200 &&data.body.length!=0){
              ...
              if(data.body.length == 1){
                ...
                if(_this.gameIntro[indexA].code =="PCDD"){
                ...
                if(_this.gameIntro[indexA].code =="hk6"){
    ...
    // },
      ...
      // if (!localStorage.isTransfer && !localStorage.app_flag && !localStorage.tipsContent) {
			...
			if(localStorage.config) {
              ...
              if(this.datas.onlineConfigure == "onlineCustomerServiceUrl") {
                localStorage.setItem("onlineCustomerServiceUrl", this.datas.onlineSys_config1);
              }

              this.logoPic = this.datas.wapPageLogoConfigure;

              //首页title
              if(data.body.webName) {
                if(this.baseIndex==1){
                  this.headTit = data.body.webName;
                  this.webName = data.body.webName;
                }
            ...
            if(data.body.webName) {
              ...
              $("head title").html(_this.headTit);
						...
						if(localStorage.app_flag != undefined) {
							...
							if(
								version != data.body.appCurrentVersion && !localStorage.versionNewCode
							) {
