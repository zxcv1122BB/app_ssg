...
var workerFn = {
	//比较数组中的数，找出最小
	findListMin: function(list) { //[4,5,6,8]
		var min = 0;
		if(list != "") {
	...
	//找到数组中最小的几个数
	findListTwoMin: function(list, index) {  //[4,5,9,6] 1

		var minList = [],
			min = 1;
		list.sort(function(a, b) {  // [4,5,6,9]
			return a - b;
		});
		for(var j = 0; j < index; j++) { // index = 1;
			minList.push(list[j]); //minList = [4]
		}
		for(var j = 0, len = minList.length; j < len; j++) { // minList.length =1
			min = min * minList[j]; //min = 1*4
		}
		return min; // 4
	},
			...
			switch(playType) {
				...
				case 2: //让球胜平负和胜平负
					for(var j = 0, len = dataList.length; j < len; j++) {
						...
						for(var l=0;l<obj.spf_rqspf.length;l++){
							var item=obj.spf_rqspf[l],itemOdds=obj.spf_rqspfOdds[l];
							if(item<3){
				...
				default:
					for(var j = 0, len1 = dataList.length; j < len1; j++) {
						...
						if(obj.selectRQ == ""&&obj.selectBrq == ""){obj.rqNum=0}
						if(obj.selectRQ == "" && obj.selectBrq != "") {
							var firstMinList = [],
								firstMaxList = [];
							for(var k = 0, len2 = obj.selectBrq.length; k < len2; k++) {
			...
			switch(indexList[i]) {
				...
				case 7:
					...
					for(var j = 0, len = lenList.length; j < len - 6; j++) {
						var k = j + 1, l = j + 2, m = j + 3, n = j + 4, q = j + 5, w = j + 6;
						do {
							do {
								do {
									do {
